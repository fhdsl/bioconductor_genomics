...
For manipulating the `SummarizedExperiment` object, we will use the `tidySummarizedExperiment` package to subset it - essentially this package lets us use `dplyr` on the objects
...
#| eval: false
GSE96870 |>
    ggplot(aes(y=log2(counts + 1), group=.sample, fill=------)) +
    geom_boxplot() +
    theme_bw()
```
